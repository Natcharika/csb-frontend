...
function DepartmentHeadScoreCSB01() {
  ...
  const [data, setData] = useState([
    { id: 1, name: "คะแนนรวม", fullscores: "90", score: "" },
  ]);
  ...
  useEffect(() => {
    ...
    const filteredProjects = mockProjects.filter(
      (project) => project.Er_CSB02 > 0 && project.Er_CSB02_status !== "ผ่าน"
    );
  ...
  useEffect(() => {
    if (selectedProject) {
      const selectedProjectData = projects.find(
        (project) => project.Er_Pname === selectedProject.Er_Pname
      );
        ...
        setData([
          {
            ...
            name: "คะแนนรวม",
            fullscores: "90",
            score: selectedProjectData.Er_CSB02,
          },
        ]);
      }
    }
  ...
  const handleSubmit = () => {
    ...
    setProjects((prevProjects) =>
      prevProjects.filter((p) => p.Er_Pname !== selectedProject.Er_Pname)
    );
