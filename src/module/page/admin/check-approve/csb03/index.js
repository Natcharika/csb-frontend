...
export default function CheckApproveCSB02() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter projects where status.CSB03.activeStatus is 2 and status.CSB03.status is "approved"
      const filteredProjects = projectsData
        .filter(
          (project) =>
            project.status?.CSB03?.activeStatus === 2 &&
            project.status?.CSB03?.status === "approved"
        )
        .map((item) => ({
